...
function findLunarDate(jd: number, ly: LunarDate[]): LunarDate {
  if (jd > LAST_DAY || jd < FIRST_DAY || ly[0].jd > jd) {
    return { day: 0, month: 0, year: 0, leap: 0, jd };
  }
  let i = ly.length - 1;
  while (jd < ly[i].jd) {
    i--;
  ...
  return { day: ly[i].day + off, month: ly[i].month, year: ly[i].year, leap: ly[i].leap, jd };
}

export function convertSolar2Lunar(dd: number, mm: number, yyyy: number): { day: number; month: number; year: number; leap: boolean } {
  if (yyyy < 1900 || yyyy > 2199) {
    return { day: dd, month: mm, year: yyyy, leap: false };
  }
  ...
  const res = findLunarDate(jd, ly);
  return { day: res.day, month: res.month, year: res.year, leap: res.leap === 1 };
}
